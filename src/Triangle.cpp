...
bool Triangle::intersect(const Ray &ray, double t_min, double &t_max, Vector3d &n) const {
    ...
    Vector3d x = A.colPivHouseholderQr().solve(b);  // does the compiler replace vertices.col(0) with a variable?
    double a = x(0) + x(1);
    if (0 < x(0) && x(0) < 1 &&
        0 < x(1) && x(1) < 1 &&
        0 < a    && a    < 1 &&
...
void Triangle::get_normal(const Vector3d &l, Vector3d &n) const {
}
