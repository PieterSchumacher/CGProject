...
class Light {
  public:
    ...
    static const unsigned char sample_rate = 1;
    ...
    static auto V(const Vector3d &p, const Vector3d &p_prime) -> bool {
        Vector3d wo = p_prime - p;
        return !find_intersection({p+wo.normalized()*1e-11, wo.normalized()}, wo.norm()-1e-10);
    }
